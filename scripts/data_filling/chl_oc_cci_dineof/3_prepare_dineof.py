...
derived_dir = 'derived/data_filling/chl_oc_cci'
dineof_dir = os.path.join(derived_dir, 'dineof')
chl_f = os.path.join(derived_dir, 'chl.mat')
mask_f = os.path.join(derived_dir, 'mask.mat')
dineof_f = os.path.join(dineof_dir, 'dineof.init')
average_f = os.path.join(derived_dir, 'chl_average.mat')

chl = sio.loadmat(chl_f)['chl']
mask = sio.loadmat(mask_f)['mask']
chl = ma.masked_invalid(chl)
chl = np.log(chl)
chl_average = chl.mean(axis=2)
data = chl - np.tile(chl_average[:,:,np.newaxis], (1,1,685))
mask = mask == 1

if not os.path.exists(dineof_dir):
    os.makedirs(dineof_dir)

sio.savemat(average_f, {'chl':chl_average.data})

genParamFile( params, dineof_f )

dineof(data, mask, dineof_f)
