...
dir.create('derived/EDA/cdom/weekly_maps/', recursive=TRUE)
load('data/common/shapefiles/red_sea_outside.Rdata')

FILE <- list.files('data/cdom/clean', full.names=TRUE, pattern='*.grd')
...
for (f in FILE) {
  ...
  p <- p + ggtitle(paste('CDOM around Arabia the week of the', d))

  fn <- paste('derived/EDA/cdom/weekly_maps/', d, '.png', sep='')
  ggsave(fn, plot=p)
}
