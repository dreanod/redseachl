...
outputDir <- 'derived/EDA/chl/regional_averages'
dir.create(outputDir, recursive=TRUE)

FILES <- list.files('data/chl/clean', full.names=TRUE, pattern='*.grd')
...
p <- p + geom_line()
p <- p + ylab('CHL (mg/m^3)')
p <- p + ggtitle('CHL average over NRS (MODIS)')
...
p <- p + geom_line()
p <- p + ylab('CHL (mg/m^3)')
p <- p + ggtitle('CHL average over NCRS (MODIS)')
...
p <- p + geom_line()
p <- p + ylab('CHL (mg/m^3)')
p <- p + ggtitle('CHL average over SCRS (MODIS)')
...
p <- p + geom_line()
p <- p + ylab('CHL (mg/m^3)')
p <- p + ggtitle('CHL average over SRS (MODIS)')
