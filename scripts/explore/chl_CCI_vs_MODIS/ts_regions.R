...
p <- p + ggtitle('average CHL in NRS by data product')
fn <- paste(outputDir, '/ts_NRS_mean.png', sep='')
ggsave(plot=p, filename=fn)

p <- ggplot(subset(df, region=='NCRS'), aes(x=date, y=chl, colour=product))
p <- p + geom_line() + geom_point()
p <- p + scale_x_date(labels=dlabels, breaks=drange)
p <- p + ggtitle('average CHL in NCRS by data product')
fn <- paste(outputDir, '/ts_NCRS_mean.png', sep='')
ggsave(plot=p, filename=fn)

p <- ggplot(subset(df, region=='SCRS'), aes(x=date, y=chl, colour=product))
p <- p + geom_line() + geom_point()
p <- p + scale_x_date(labels=dlabels, breaks=drange)
p <- p + ggtitle('average CHL in SCRS by data product')
fn <- paste(outputDir, '/ts_SCRS_mean.png', sep='')
ggsave(plot=p, filename=fn)

p <- ggplot(subset(df, region=='SRS'), aes(x=date, y=chl, colour=product))
p <- p + geom_line() + geom_point()
p <- p + scale_x_date(labels=dlabels, breaks=drange)
p <- p + ggtitle('average CHL in SRS by data product')
fn <- paste(outputDir, '/ts_SRS_mean.png', sep='')
ggsave(plot=p, filename=fn)