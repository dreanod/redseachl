...
dir.create('derived/EDA/par/weekly_maps/', recursive=TRUE)
load('data/common/shapefiles/red_sea_outside.Rdata')

FILE <- list.files('data/par/clean', full.names=TRUE, pattern='*.grd')
...
for (f in FILE) {
  ...
  p <- p + ggtitle(paste('PAR (Einstein/m^2.day) around Arabia the week of the', d))

  fn <- paste('derived/EDA/par/weekly_maps/', d, '.png', sep='')
  ggsave(fn, plot=p)
}
