...
dir.create('derived/EDA/poc/weekly_maps/', recursive=TRUE)
load('data/common/shapefiles/red_sea_outside.Rdata')

FILE <- list.files('data/poc/clean', full.names=TRUE, pattern='*.grd')
...
for (f in FILE) {
  ...
  p <- p + scale_fill_gradientn(colours=rev(brewer.pal(7, 'Spectral')),
                                            limits=c(15, 1e4),
                                name='POC (mg/m^3)', trans='log',
                                breaks=c(10,50,100,500,1000,5000))
  p <- p + coord_cartesian() 
  p <- p + geom_polygon(aes(x=long, y=lat, group=group), red.sea.outside,
                        fill='white', contour='black')
  p <- p + ggtitle(paste('POC around Arabia the week of the', d))

  fn <- paste('derived/EDA/poc/weekly_maps/', d, '.png', sep='')
  ggsave(fn, plot=p)
}
