...
outputDir <- 'derived/EDA/chl_oc_cci/climatology_maps'
...
p <- p + ggtitle('yearly CHL average (OC-CCI 1997-2012)')

fn <- paste(outputDir, '/yearly_average.png', sep='')
ggsave(fn, plot=p)

# seasonal climatologies (summer/winter)
...
p <- p + geom_polygon(aes(x=long, y=lat, group=group), red.sea.outside,
                      fill='white', contour='black')
p <- p + ggtitle('winter CHL average (OC-CCI 1997-2012)')
...
p <- p + geom_polygon(aes(x=long, y=lat, group=group), red.sea.outside,
                      fill='white', contour='black')
p <- p + ggtitle('summer CHL average (OC-CCI 1997-2012)')
