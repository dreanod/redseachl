...
FILES <- list.files('data/chl_oc_cci/clean', pattern='*.grd', full.names=TRUE)
...
drange <- seq(as.Date('1998-01-01'), as.Date('2011-01-01'), 'year')
dlabels <- as.character(seq(1998, 2011))
p <- ggplot(df, aes(x=date, y=missing)) + geom_line()
p <- p + scale_x_date(breaks=drange, labels=dlabels)
p <- p + ggtitle('ratio of CHL missing values in the Red Sea (OC-CCI)')
fn <- paste(outputDir, '/missing_val_red_sea_ts.png', sep='')
...
p <- p + geom_line() 
p <- p + scale_x_date(breaks=drange, labels=dlabels)
p <- p + ggtitle('ratio of missing values in Red Sea (climatology from 1997 to 2012, OC-CCI data)')
fn <- paste(outputDir, '/missing_val_red_sea_climatology.png', sep='')
ggsave(fn, plot=p)
